...
public class LinkedList<T extends Comparable> {
    ...
    public int searchAndMoveToBeginning(int value) {
        int accesses = 0;
        Node<T> prev = null;
        Node<T> temp = head;
        while (temp != null) {
            accesses++;
            if (temp.value.compareTo(value) == 0) {
                //Value is found and move to the beginning of the Linked list
                if (prev != null) {
                    prev.next = temp.next;
                    temp.next = head;
                    head = temp;
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (String value : values) {
                    ...
                    int accesses = searchAndMoveToBeginning(intValue);
                    if (accesses != -1) {
